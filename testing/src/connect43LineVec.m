function [candidates, subset] = connect43LineVec(image, heatMaps, param, vis)
...
for k = 1:size(mapIdx,1)
    ...
    if(nA ==0 && nB ==0)
        continue;
    elseif nA ==0
        temp = zeros(nB,kpt_num); 
        temp(:, limbSeq(k, 2)) = candB(:,4);
        temp(:,end) = ones(nB,1);
        temp(:,end-1) = candB(:,3);
        subset = [subset; temp];
        continue;
...
%                 end
            ...
            if mid_score(1) > 0 && suc_ratio > 0.7 %second threshold
                score = sum(mid_score);
...
for i=1:size(subset,1)
    if (subset(i,end)<param.thre3) || (subset(i,end-1)/subset(i,end)<param.thre4)
        deleIdx = [deleIdx;i];
    end
end
subset(deleIdx,:) = [];
...
if vis == 1
    ...
    for num = 1:size(subset,1)
        ...
        for i = 1:14
            ...
            image = insertShape(image, 'FilledCircle', [X Y 8], 'Color', joint_color(i,:)); 
        end
    end
    imshow(image), hold on;
    
    for idx = 1:size(candidates,1)
        if candidates(idx,4)<15
            %plot(candidates(idx,1), candidates(idx,2), 'Color', joint_color(candidates(idx,4),:), 'Marker', '.', 'Linewidth', 10);
            %plot(candidates(idx,1), candidates(idx,2), 'rx', 'Linewidth', 2);
    ...
    for num = 1:size(subset,1)
        %imshow(image);
        for i = 1:size(limbSeq,1)
            index = subset(num,limbSeq(i,1:2));
            if sum(index==0)>0 %|| i == 8 || i == 9 || i == 12
                continue;
            end
            X = candidates(index,1);
            Y = candidates(index,2);
